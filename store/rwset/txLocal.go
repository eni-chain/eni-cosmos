...
func (store *TxExecutionStore) Write() {
	panic("TxExecutionStore does not support Write")
}

// CacheWrap function is not entered when executing a transaction
func (store *TxExecutionStore) CacheWrap() types.CacheWrap {
	return store.parent.CacheWrap()
}

// CacheWrapWithTrace function is not entered when executing a transaction
func (store *TxExecutionStore) CacheWrapWithTrace(w io.Writer, tc types.TraceContext) types.CacheWrap {
	return store.parent.CacheWrapWithTrace(w, tc)
}

// Iterator function is not entered when executing a transaction
func (store *TxExecutionStore) Iterator(start, end []byte) types.Iterator {
	return store.parent.Iterator(start, end)
}

// ReverseIterator function is not entered when executing a transaction
