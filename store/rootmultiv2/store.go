...
import (
	...
	commonerrors "github.com/eni-chain/eni-db/common/errors"
	"github.com/eni-chain/eni-db/config"
	"github.com/eni-chain/eni-db/proto"
	"github.com/eni-chain/eni-db/sc"
...
func (rs *Store) flush() error {
	...
	for key := range rs.ckvStores {
		...
		if commitStore, ok := store.(*commitmentv2.Store); ok {
...
func (rs *Store) CacheMultiStoreWithVersion(version int64) (types.CacheMultiStore, error) {
	...
	if rs.ssStore != nil {
		for k, store := range rs.ckvStores {
			if store.GetStoreType() == types.StoreTypeIAVL {
				stores[k] = statev2.NewStore(rs.ssStore, k, version)
			}
		}
	}
...
func (rs *Store) CacheMultiStoreForExport(version int64) (types.CacheMultiStore, error) {
	...
	for k, store := range rs.ckvStores {
		if store.GetStoreType() == types.StoreTypeIAVL {
			...
			stores[k] = commitmentv2.NewStore(tree, rs.logger)
		}
	}
	rs.mtx.RUnlock()
...
func (rs *Store) loadCommitStoreFromParams(key types.StoreKey, params storeParams) (types.CommitKVStore, error) {
	...
	case types.StoreTypeIAVL:
		...
		return types.CommitKVStore(commitmentv2.NewStore(tree, rs.logger)), nil
...
func (rs *Store) Query(req *types.RequestQuery) (*types.ResponseQuery, error) {
	...
	if !req.Prove && rs.ssStore != nil {
		...
		store = types.Queryable(statev2.NewStore(rs.ssStore, types.NewKVStoreKey(storeName), version))
	} else {
...
func (rs *Store) AddListeners(_ []types.StoreKey) {
	return
}
